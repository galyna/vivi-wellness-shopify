...
export default function PaymentStep() {
  ...
  const validate = () => {
    ...
    if (!/^\d{16}$/.test(form.cardNumber || "")) newErrors.cardNumber = "Введіть 16 цифр";
    if (!form.cardName?.trim()) newErrors.cardName = "Введіть ім&apos;я на картці";
    if (!/^\d{2}\/\d{2}$/.test(form.cardExpiry || "")) newErrors.cardExpiry = "MM/YY";
    if (!/^\d{3,4}$/.test(form.cardCvc || "")) newErrors.cardCvc = "3-4 цифри";
    setErrors(newErrors);
    ...
    <form className="space-y-6" onSubmit={e => { e.preventDefault(); handleNext(); }}>
      ...
      <div className="flex gap-4 mb-4">
        <label className={`flex items-center gap-2 px-3 py-2 rounded border cursor-pointer ${form.method === "card" ? "border-coral bg-coral/10" : "border-gray-300"}`}>
          <input type="radio" name="method" checked={form.method === "card"} onChange={() => setForm(f => ({ ...f, method: "card" }))} /> Карткою онлайн
        </label>
        <label className={`flex items-center gap-2 px-3 py-2 rounded border cursor-pointer ${form.method === "cod" ? "border-coral bg-coral/10" : "border-gray-300"}`}>
          <input type="radio" name="method" checked={form.method === "cod"} onChange={() => setForm(f => ({ ...f, method: "cod" }))} /> При отриманні
        </label>
      </div>
      {form.method === "card" && (
      ...
      <div className="flex gap-2 mt-8">
        <button type="button" onClick={() => setStep(2)} className="flex-1 py-2 rounded-full bg-gray-200 text-gray-700 font-semibold hover:bg-gray-300">Назад</button>
        <button type="submit" className="flex-1 py-2 rounded-full bg-coral text-white font-bold hover:bg-coral/90">Далі</button>
      </div>
    </form>
  );
} 