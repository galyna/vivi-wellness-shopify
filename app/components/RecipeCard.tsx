...
const RecipeCard = ({ id, title, ingredients, category, image }: RecipeCardProps) => (
  <div className="bg-white rounded-2xl shadow-md p-6 flex flex-col items-center gap-2 w-full max-w-xs hover:shadow-lg transition">
    ...
    <Link href={`/recipes/${id}`} className="mt-3 px-4 py-2 rounded-full bg-coral text-white font-bold shadow hover:bg-neon transition text-sm">View Recipe</Link>
  </div>
);
